...
class AutoTradingApp:	public EWrapper,
	public IAppControl
{
public:
	...
	{
	public:
		Scope(IAppControl *appControl);
		~Scope();

		IAppControl *operator ->() {return m_appControl;}
		bool operator ! () { return m_appControl != nullptr; }

	private:
		IAppControl *m_appControl;
	};

	virtual bool Initialize(InfoOutputUtil::EOutputType outputType);
	virtual bool run();
...
private:
	...
	friend Singleton<AutoTradingApp>;

	EClientSocket	m_eClientSocket;
	EReaderOSSignal	m_eReaderOSSignal;
	std::unique_ptr<EReader>	m_pReader;

};


typedef Singleton<AutoTradingApp> AutoTradingAppSingleton;